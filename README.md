...
```text
Все тесты проекта должны завершаться статусом OK.
```
...
- Светлая и тёмная тема реализованы и переключаются через профиль пользователя.
...
6. Добавить страницу управления и выгрузки персональных данных.
7. Добавить REST API для мобильного приложения.
8. Подключить настоящий LLM-отчет с guardrails и безопасными ограничениями.


## Финальные возможности проекта

- регистрация, вход и выход пользователя;
- страница политики обработки персональных данных `/privacy/`;
- согласие на обработку данных при регистрации;
- выбор пола при регистрации и в профиле;
- просмотр и скрытие пароля в формах;
- светлая и тёмная тема;
- создание, просмотр, редактирование и удаление записей дневника;
- ограничение редактирования записи по времени;
- история записей;
- статистика и графики;
- CSV-экспорт данных;
- rule-based AI-отчёт без зависимости от внешнего API;
- карточки дыхательных практик;
- интерактивный таймер выполнения практики;
- фоновая спокойная музыка во время практики;
- demo-data для локальной проверки проекта.


## Музыка в дыхательных практиках

Для фоновой музыки во время выполнения практики используется локальный файл:

```text
static/audio/practice_calm.mp3
```

Источник: Alex Productions — Ambient Music Nature, Chosic.

Проект является учебным. Перед публичным или коммерческим использованием необходимо отдельно проверить условия лицензии аудиофайла.
